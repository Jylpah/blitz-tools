...
class ThrottledClientSession(aiohttp.ClientSession):
    ...
    def set_rate_limit(self, rate_limit: float = 0) -> float:
        ...
        assert (
            isinstance(rate_limit, (int, float)) and rate_limit >= 0
        ), "rate_limit has to be type of 'float' >= 0"
...
def verbose(msg="", id=None) -> bool:
    """Print a message"""
    return _print_log_msg(
        "", msg, exception=None, id=id, print_msg=(_log_level >= VERBOSE)
    )
...
def verbose_std(msg="", id=None) -> bool:
    """Print a message"""
    return _print_log_msg(
        "", msg, exception=None, id=id, print_msg=(_log_level >= NORMAL)
    )
...
def warning(msg="", id=None, force: bool = False) -> bool:
    ...
    return _print_log_msg(
        "", "Warning: " + msg, None, id, print_msg=(force or (_log_level >= NORMAL))
    )
...
def log(msg="", id=None, exception=None) -> bool:
    """print a conditional debug message"""
    return _print_log_msg(
        "LOG", msg=msg, exception=exception, id=id, print_msg=(_log_level >= DEBUG)
    )
...
def set_progress_bar(
    heading: str, max_value: int, step: int = None, slow: bool = False, id: str = None
):
    ...
    if slow:
        ...
        _progress_obj = IncrementalBar(
            heading, max=max_value, suffix="%(index)d/%(max)d %(percent)d%%"
        )
    _progress_i = 0
...
async def save_JSON(
    filename: str, json_data: dict, sort_keys=False, pretty=True
) -> bool:
    ...
    try:
        ...
        async with aiofiles.open(filename, "w", encoding="utf8") as outfile:
            if pretty:
                await outfile.write(
                    json.dumps(
                        json_data, ensure_ascii=False, indent=4, sort_keys=sort_keys
                    )
                )
            else:
                await outfile.write(
                    json.dumps(json_data, ensure_ascii=False, sort_keys=sort_keys)
                )
            return True
    except Exception as err:
...
async def get_url_JSON(
    session: aiohttp.ClientSession, url: str, chk_JSON_func=None, max_tries=MAX_RETRIES
) -> dict:
        ...
        try:
            async with session.get(url) as resp:
                ...
                if retry == max_tries:
                    break
                debug(
                    "Retrying URL [" + str(retry) + "/" + str(max_tries) + "]: " + url
                )
...
def get_JSON_value(
    json, key: str | None = None, keys: list | None = None, keypath: str | None = None
):
    if keys is None:
        if key is not None:
            keys = key.split(".")
    ...
    if type(json) == dict:
        if key in json:
            return get_JSON_value(
                json[key], keys=keys, keypath=get_JSON_keypath(keypath, key)
            )
        else:
    ...
    if type(json) == list:
        ...
        try:
            return get_JSON_value(
                json[ndx], keys=keys, keypath=get_JSON_keypath(keypath, key)
            )
        except IndexError:
            raise KeyError(
                "JSON array index out of range: " + get_JSON_keypath(keypath, key)
            )
...
class WG:
    ...
    _DEFAULT_WG_APP_ID = "81381d3f45fa4aa75b78a7198eb216ad"
    ...
    SQL_TANK_STATS_UPDATE = (
        "REPLACE INTO "
        + SQL_TANK_STATS_TBL
        + "(account_id, tank_id, update_time, stats) VALUES(?,?,?,?)"
    )

    # PLAYER_STATS
    SQL_PLAYER_STATS_TBL = "player_stats"

    SQL_PLAYER_STATS_CREATE_TBL = (
        "CREATE TABLE IF NOT EXISTS "
        + SQL_PLAYER_STATS_TBL
    ...
    SQL_PLAYER_STATS_UPDATE = (
        "REPLACE INTO "
        + SQL_PLAYER_STATS_TBL
        + "(account_id, update_time, stats) VALUES(?,?,?)"
    )

    SQL_PLAYER_STATS_CACHED = (
        "SELECT * FROM "
        + SQL_PLAYER_STATS_TBL
        + " WHERE account_id = ? AND update_time > ?"
    )

    # PLAYER_ACHIEVEMENTS
    SQL_PLAYER_ACHIEVEMENTS_TBL = "player_achievements"

    SQL_PLAYER_ACHIEVEMENTS_CREATE_TBL = (
        "CREATE TABLE IF NOT EXISTS "
        + SQL_PLAYER_ACHIEVEMENTS_TBL
        + """ ( account_id INTEGER PRIMARY KEY, 
                                    update_time INTEGER NOT NULL, 
                                    stats TEXT)"""
    )

    SQL_PLAYER_ACHIEVEMENTS_COUNT = (
        "SELECT COUNT(*) FROM " + SQL_PLAYER_ACHIEVEMENTS_TBL
    )

    SQL_PLAYER_ACHIEVEMENTS_CACHED = (
        "SELECT * FROM "
        + SQL_PLAYER_ACHIEVEMENTS_TBL
        + " WHERE account_id = ? AND update_time > ?"
    )

    SQL_PLAYER_ACHIEVEMENTS_UPDATE = (
        "REPLACE INTO "
        + SQL_PLAYER_ACHIEVEMENTS_TBL
        + "(account_id, update_time, stats) VALUES(?,?,?)"
    )
    ...
    TANK_TYPE_ID = {"lightTank": 0, "mediumTank": 1, "heavyTank": 2, "AT-SPG": 3}
        ...
        if self.WG_app_id is not None:
            ...
            if self.global_rate_limit:
                self.session = ThrottledClientSession(
                    rate_limit=rate_limit, headers=headers
                )
            else:
                self.session = dict()
                for server in list(self.URL_WG_SERVER)[
                    :4
                ]:  # China (5th) server is unknown, thus excluded
                    self.session[server] = ThrottledClientSession(
                        rate_limit=rate_limit, headers=headers
                    )
    ...
    def chk_JSON_status(cls, json_resp: dict) -> bool:
        try:
            if (
                (json_resp is None)
                or ("status" not in json_resp)
                or (json_resp["status"] is None)
            ):
                return False
                ...
                if "error" in json_resp:
                    ...
                    if ("message" in json_resp["error"]) and (
                        json_resp["error"]["message"] is not None
                    ):
                        error_msg = error_msg + ": " + json_resp["error"]["message"]
                    if ("value" in json_resp["error"]) and (
                        json_resp["error"]["value"] is not None
                    ):
    ...
    def get_tanks_by_tier(self, tier: int) -> list[int]:
        """Returns tank_ids by tier"""
        res: list[int] = list()
        try:
            for tank in self.tanks:
                if tank.tier == tier:
                    res.append(tank.tank_id)
        except KeyError as err:
    ...
    def get_url_clan_info(self, server: str, clan_id: int) -> str:
        try:
            if server is None:
                return None
            return (
                self.URL_WG_SERVER[server]
                + self.URL_WG_CLAN_INFO
                + self.WG_app_id
                + "&clan_id="
                + str(clan_id)
            )
            ...
            if (server is None) or (server.lower() not in WG.ACCOUNT_ID_SERVER.keys()):
                error(
                    "No server name or invalid server name given: " + server
    ...
    def get_url_player_tanks_stats(
        self, account_id: int, tank_ids: list[int] = [], fields: list[str] = []
    ) -> str:
    ...
    def get_url_player_achievements(
        self, account_ids: list, fields: str = "max_series"
    ) -> str:
        try:
            ...
            return (
                self.URL_WG_SERVER[server]
                + self.URL_WG_PLAYER_ACHIEVEMENTS
                + self.WG_app_id
                + "&account_id="
                + account_ids_str
                + field_str
        ...
        if self.global_rate_limit:
            ...
            for server in self.session:
                verbose_std(
                    ...
                    + server
                    + ": "
                    + self.session[server].get_stats_str()
                )

    async def get_url_JSON(
        self, url: str, chk_JSON_func=None, max_tries=MAX_RETRIES
    ) -> dict:
    ...
    async def get_player_tank_stats(
        self, account_id: int, tank_ids=[], fields=[], cache=True, cache_only=False
    ) -> dict:
    ...
    async def get_player_stats(
        self, account_id: int, fields=[], cache=True, cache_only=False
    ) -> dict:
    ...
    async def get_player_achievements(
        self, account_ids: list, fields=[], cache=True
    ) -> dict:
        ...
        try:
            ...
            if cache:
                ...
                for account_id in account_ids:
                    try:
                        stats[
                            str(account_id)
                        ] = await self.get_cached_player_achievements(
                            account_id, fields
                        )
            ...
            if (json_data is not None) and ("data" in json_data):
                ...
                for account_id in json_data["data"].keys():
                    ...
                    if cache:
                        await self.save_player_achievements(
                            int(account_id), json_data["data"][account_id]
                        )
                return stats
        except Exception as err:
            error(exception=err)
    ...
    def get_tank(self, tank_id: int) -> WGTank:
        return self.tanks[tank_id]

    def get_tank_tier(self, tank_id: int) -> int:
        tank: WGTank = self.get_tank(tank_id)
        if tank.tier is not None:
            return int(tank.tier)
        raise ValueError(f"could not find tier for tank_id={tank_id}")

    def get_tank_name(self, tank_id: int) -> str:
        tank: WGTank = self.get_tank(tank_id)
        if tank.name is not None:
            return tank.name
        raise ValueError(f"could not get name for tank_id={tank_id}")
    ...
    def get_tank_type(self, tank_id: int) -> str:
        tank: WGTank = self.get_tank(tank_id)
        if tank.type is not None:
            return str(tank.type)
        raise ValueError(f"could not get type for tank_id={tank_id}")

    def get_tank_nation(self, tank_id: int) -> str:
        tank: WGTank = self.get_tank(tank_id)
        if tank.nation is not None:
            return str(tank.nation.name)
        raise ValueError(f"could not get nation for tank_id={tank_id}")

    def is_premium(self, tank_id: int) -> bool:
        tank: WGTank = self.get_tank(tank_id)
        return tank.is_premium
        ...
        try:
            ...
            if is_debug():
                async with self.cache.execute(WG.SQL_PLAYER_STATS_COUNT) as cursor:
                    debug(
                        "Cache contains: "
                        + str((await cursor.fetchone())[0])
                        + " player stat records"
                    )
                async with self.cache.execute(
                    WG.SQL_PLAYER_ACHIEVEMENTS_COUNT
                ) as cursor:
                    debug(
                        "Cache contains: "
                        + str((await cursor.fetchone())[0])
            ...
            async with self.cache.execute(WG.SQL_CHECK_TABLE_EXITS, (table,)) as cursor:
                if (await cursor.fetchone()) is not None:
                    ...
                    await self.cache.execute(
                        WG.SQL_PRUNE_CACHE.format(table, NOW() - grace_time)
                    )
                    await self.cache.commit()
        return None
        ...
        try:
            ...
            if stats_data is not None:
                for stat in stats_data:
                    ...
                    await self.cache.execute(
                        WG.SQL_TANK_STATS_UPDATE,
                        (account_id, tank_id, update_time, json.dumps(stat)),
                    )
                    tank_ids.discard(tank_id)
            # no stats found => Add None to mark that
            for tank_id in tank_ids:
                await self.cache.execute(
                    WG.SQL_TANK_STATS_UPDATE, (account_id, tank_id, update_time, None)
                )
            await self.cache.commit()
    ...
    async def store_player_stats(self, key: list, stats_data: list, update_time: int):
        ...
        try:
            ...
            if stats_data is not None:
                await self.cache.execute(
                    WG.SQL_PLAYER_STATS_UPDATE,
                    (account_id, update_time, json.dumps(stats_data)),
                )
            else:
                await self.cache.execute(
                    WG.SQL_PLAYER_STATS_UPDATE, (account_id, update_time, None)
                )
            await self.cache.commit()
    ...
    async def store_player_achievements(
        self, key: list, stats_data: list, update_time: int
    ):
        """Save player stats into cache"""
        assert self.cache is not None, "cache must be initiated"
        ...
        try:
            ...
            if stats_data is not None:
                await self.cache.execute(
                    WG.SQL_PLAYER_ACHIEVEMENTS_UPDATE,
                    (account_id, update_time, json.dumps(stats_data)),
                )
            else:
                await self.cache.execute(
                    WG.SQL_PLAYER_ACHIEVEMENTS_UPDATE, (account_id, update_time, None)
                )
            await self.cache.commit()
    ...
    async def get_cached_tank_stats(
        self, account_id: int, tank_ids: list[int], fields: list
    ):
        assert self.cache is not None, "cache must be initiated"
        ...
        try:
            ...
            if len(tank_ids) > 0:
                sql_query = (
                    "SELECT * FROM "
                    + WG.SQL_TANK_STATS_TBL
                    + " WHERE account_id = ? AND update_time > ? AND tank_id IN ("
                    + ",".join([str(x) for x in tank_ids])
                    + ")"
                )
            else:
                sql_query = (
            ...
            async with self.cache.execute(
                sql_query, [account_id, NOW() - WG.CACHE_GRACE_TIME]
            ) as cursor:
    ...
    async def get_cached_player_stats(self, account_id, fields):
        try:
            ...
            async with self.cache.execute(
                WG.SQL_PLAYER_STATS_CACHED, [account_id, NOW() - WG.CACHE_GRACE_TIME]
            ) as cursor:
    ...
    async def get_cached_player_achievements(self, account_id, fields):
        try:
            ...
            async with self.cache.execute(
                WG.SQL_PLAYER_ACHIEVEMENTS_CACHED,
                [account_id, NOW() - WG.CACHE_GRACE_TIME],
            ) as cursor:
                row = await cursor.fetchone()
                # debug('account_id: ' + str(account_id) + ': 1')
                if row is None:
...
class WoTinspector:
    ...
    async def get_tankopedia(self, filename="tanks.json"):
        ...
        async with self.session.get(self.URL_TANK_DB) as r:
            if r.status == 200:
                ...
                print(
                    "Error: Could not get valid HTTPS response. HTTP: " + str(r.status)
                )
                sys.exit(1)
            tanks = {}
            n = 0
            p = re.compile(
                '\\s*(\\d+):\\s{"en":"([^"]+)",.*?"tier":(\\d+), "type":(\\d), "premium":(\\d).*'
            )
            ...
            with open(filename, "w") as outfile:
                outfile.write(
                    json.dumps(
                        tankopedia, ensure_ascii=False, indent=4, sort_keys=False
                    )
                )
            return None

    async def get_replay_JSON(self, replay_id: str):
        json_resp = await get_url_JSON(
            self.session, self.URL_REPLAY_INFO + replay_id, chk_JSON_func=None
        )
        try:
    ...
    async def post_replay(
        ...
        try:
            ...
            url = self.URL_REPLAY_UL + urllib.parse.urlencode(
    ...
    def chk_JSON_replay(cls, json_resp) -> bool:
        ...
        try:
            if (
                ("status" in json_resp)
                and json_resp["status"] == "ok"
                and (get_JSON_value(json_resp, key="data.summary.exp_base") is not None)
            ):
                debug("JSON check OK")
                return True
        except KeyError as err:
