...
class StatFunc:
    ...
    STAT_FUNC = {
        "tank_tier": [
            "get_stat_id_tank_tier",
            "get_db_tank_tier_stats",
            "get_wg_tank_tier_stats",
            "WR at tier",
        ],
        "tank": [
            "get_stat_id_tank",
...
histogram_fields = {
    "wins": [
        "Win rate",
        [0, 0.35, 0.40, 0.45, 0.48, 0.5, 0.52, 0.55, 0.60, 0.65, 0.70, 1],
        1,
        "{:2.0%}",
    ],
    "damage_dealt": [
    ...
    "battles": [
        "Battles",
        [0, 1000, 2500, 5000, 7000, 10e3, 15e3, 25e3, 50e3, 5e7],
        0.001,
        "{:.0f}k",
    ],  # battles is a mandatory stat to include
}
...
class BattleCategorizationList:
    ...
    _categorizations = {
        ...
        "mastery_badge": [
            "Battle Medal",
            "category",
            ["-", "3rd Class", "2nd Class", "1st Class", "Mastery"],
        ],
        "team_result": ["Team Result", "string"],
        "player_wins": ["Player WR", "bucket", [0, 0.35, 0.45, 0.50, 0.55, 0.65], "%"],
        "player_battles": [
            "Player Battles",
            "bucket",
        ...
        "hit_rate": [
            "Player Hit Rate",
            "bucket",
            [0, 0.25, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95],
            "%",
        ],
        "pen_rate": [
            "Player Pen Rate",
        ...
        "alive": [
            "Time alive (pcs)",
            "bucket",
            [x / 100 for x in range(0, 100, 10)],
            "%",
        ],
        "time_alive": [
            "Time alive (s)",
            "bucket",
            [x * 60 for x in range(0, 8)],
            "int",
        ],
        "battle_duration": [
            "Battle Duration",
        ...
        "distance_travelled": [
            "Distance Driven",
            "bucket",
            [x * 250 for x in range(0, 20)],
            "int",
        ],
        "allies_wins": ["Allies WR", "bucket", [0, 0.35, 0.45, 0.50, 0.55, 0.65], "%"],
        "allies_battles": [
            "Player Battles",
            "bucket",
            [0, 500, 1000, 2500, 5e3, 10e3, 15e3, 25e3],
    ...
    _categorizations_stats = [
        ...
        for measure in ["wins", "battles", "damage_dealt"]
    ]
    ...
    def get_categorizations_extra(cls) -> list:
        ...
        return list(
            set(cls._categorizations.keys()) - set(cls.get_categorizations_default())
        )
        ...
        for cat in cls.get_categorizations_all():
            try:
                ...
                if ctype == "number":
                    ...
                    cat_tmp = BattleBucketCategorization(
                        cat, ctitle, cparams[0], cparams[1]
                    )
                else:
                ...
                bu.error(
                    "BattleCategorizationList(): Key not found: Category: " + cat,
                    exception=err,
                )
            except Exception as err:
    ...
    def __init__(self, cats: list):
        ...
        # ordering
        self.categorizations_list = [
            cat for cat in self._categorizations if cat in cats
        ]
        ...
        for cat in self.categorizations_list:
            try:
                ...
                if ctype == "total":
                    ...
                    self.categorizations[cat] = BattleStringCategorization(
                        cat, ctitle, cparams
                    )
                elif ctype == "category":
                    self.categorizations[cat] = BattleClassCategorization(
                        cat, ctitle, cparams[0]
                    )
                elif ctype == "bucket":
                    self.categorizations[cat] = BattleBucketCategorization(
                        cat, ctitle, cparams[0], cparams[1]
                    )
        ...
        try:
            if len(self.urls) > 0:
                ...
                for title in self.urls:
                    print(
                        ("{:" + str(3 + self.url_title_max_len) + "}").format(title)
                        + " : "
                        + self.urls[title]
                    )
        except Exception as err:
...
class BattleClassCategorization(BattleCategorization):
    ...
    def get_category(self, result: dict) -> str:
        ...
        try:
            ...
            bu.error(
                "Category: " + str(self.category_key) + " ID: " + str(cat_id),
                exception=err,
            )
        except Exception as err:
            bu.error(
...
class BattleStringCategorization(BattleCategorization):
    ...
    def get_categories(self):
        ...
        try:
            ...
            if p_number_sort.match(cats[0]) is not None:
                ndxs = sorted(
                    range(len(cats)), key=lambda k: int(cats[k][: cats[k].find(":")])
                )
...
class BattleBucketCategorization(BattleCategorization):
    ...
    def find_bucket(self, value, bucket_breaks: list):
        ...
        try:
            ...
            bu.error(
                "Key not found ndx=" + str(ndx) + ", value=" + str(value), exception=err
            )
        except Exception as err:
    ...
    def mk_category_labels(self, bucket_breaks: list, bucket_format: str) -> list:
        try:
            ...
            bu.error(
                "KeyError: ndx=" + str(ndx) + " buckets=" + str(bucket_breaks),
                exception=err,
            )
        except Exception as err:
...
class BattleCategory:
    ...
    _result_fields = {
        ...
        "pen_rate_received": [
            "Penned%",
            "Received hits pen / hits received",
            7,
            "{:7.1%}",
        ],
        "splash_rate": ["Splash%", "HE splash shots / shots hit", 7, "{:7.1%}"],
        "splash_rate_received": [
            "Splashed%",
            "HE splash shots received / hits hit",
            9,
            "{:9.1%}",
        ],
        ...
        "alive": [
            "Share live",
            "Percentage of time being alive in a battle",
            8,
            "{:8.0%}",
        ],
        ...
        "allies_wins": [
            "Allies WR",
            "Average WR of allies at the tier of their tank",
            9,
            "{:9.2%}",
        ],
        "enemies_wins": [
            "Enemies WR",
            "Average WR of enemies at the tier of their tank",
            10,
            "{:10.2%}",
        ],
        "player_battles": [
            "Player Btls",
            "Average number battles of the player",
            11,
            "{:11.0f}",
        ],
        "allies_battles": [
            "Allies Btls",
            "Average number battles of the allies",
            11,
            "{:11.0f}",
        ],
        "enemies_battles": [
            "Enemies Btls",
            "Average number battles of the enemies",
            12,
            "{:12.0f}",
        ],
        "player_damage_dealt": [
        ...
        "allies_damage_dealt": [
            "Allies Avg Dmg",
            "Average damage of the allies",
            14,
            "{:14.0f}",
        ],
        "enemies_damage_dealt": [
            "Enemies Avg Dmg",
            "Average damage of the enemies",
            15,
            "{:14.0f}",
        ],
    ...
    count_fields = ["battles", MISSING_STATS, "mastery_badge"]
    ...
    def set_fields(cls, mode: str = None):
        try:
            ...
            cls.result_fields_ratio = set(cls._result_ratios.keys()) & set(
                cls.result_fields
            )
            cls.avg_fields = (
                set(cls.result_fields)
                - set(cls._result_ratios.keys())
                - set(cls.count_fields)
            )
        ...
        try:
            for field in self.result_fields_ratio:
                if self.results[self._result_ratios[field][1]] != 0:
                    self.results[field] = (
                        self.results[self._result_ratios[field][0]]
                        / self.results[self._result_ratios[field][1]]
                    )
                else:
    ...
    def get_results(self) -> dict:
        ...
        if not self.results_ready:
            bu.error(
                "Stats have not been calculated yet. call calc_results() before get_results()"
            )
        try:
            results = dict()
    ...
    def get_results_list(self) -> list:
        ...
        try:
            ...
            for field in self.result_fields:
                results.append(
                    self._result_fields[field][3].format(self.results[field])
                )
            return results
        except KeyError as err:
            bu.error("Key not found", err)
    ...
    def print_results(self):
        if not self.results_ready:
            bu.error(
                "Stats have not been calculated yet. call calc_results() before printing()"
            )
        try:
            results = []
            for field in self.result_fields:
...
class PlayerHistogram:
    def __init__(
        self, field: str, name: str, fields: list, factor: float, cat_format: str
    ):
        self.field = field
        if self.field == "wins":
    ...
    def print(self):
        try:
            if self.results is not None:
                print(
                    "\n{:12s} | {:13s} | {:13s} | {:13s}".format(
                        self.name, "Allies", "Enemies", "TOTAL"
                    )
                )
                for cat in self.results:
                    stat = self.results[cat]
                    print(
                        "{:12s} | {:5d} ({:5.1%}) | {:5d} ({:5.1%}) | {:5d} ({:5.1%})".format(
                            cat,
                            stat["allies"],
                            stat["allies%"],
                            stat["enemies"],
...
async def main(argv):
    ...
    try:
        ...
        if os.path.isfile(FILE_CONFIG):
            ...
            try:
                if "ANALYZER" in config.sections():
                    ...
                    OPT_EXPORT_CSV_FILE = configAnalyzer.get(
                        "csv_file", OPT_EXPORT_CSV_FILE
                    )
                    OPT_EXPORT_JSON_FILE = configAnalyzer.get(
                        "json_file", OPT_EXPORT_JSON_FILE
                    )
                    res_categorizations = configAnalyzer.get("categorizations", None)
                    if res_categorizations is not None:
                        res_categorizations.replace(" ", "")
                        BattleCategorizationList.set_categorizations_default(
                            res_categorizations.split(",")
                        )
                    histogram_fields_str = configAnalyzer.get("histogram_buckets", None)
                    if histogram_fields_str is not None:
        ...
        parser = ErrorCatchingArgumentParser(
            description="Analyze Blitz replay JSON files from WoTinspector.com. Use 'upload_wotb_replays.py' to upload the replay files first."
        )
        ...
        parser.add_argument(
            ...
            choices=OPT_MODES,
            help="Select stats to print out (columns). Options: "
            + ", ".join(OPT_MODES),
        )
        parser.add_argument(
            "--extra",
            choices=BattleCategorizationList.get_categorizations_all(),
            default=None,
            nargs="*",
            help="Print extra categories: "
            + ", ".join(
                cat + "=" + BattleCategorizationList.get_categorization_title(cat)
                for cat in BattleCategorizationList.get_categorizations_all()
            ),
        )
        parser.add_argument(
            "--only_extra",
            action="store_true",
            default=False,
            help="Print only the extra categories",
        )
        parser.add_argument(
            "--hist",
            action="store_true",
            default=OPT_HIST,
            help="Print player histograms: "
            + ", ".join(histogram_fields[k][0] for k in histogram_fields),
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help="WG account_id to analyze. Replays without the account_id will be ignored.",
        )
        parser.add_argument(
            "-a",
            "--account",
            type=str,
            default=None,
            help="WG account name to analyze. Format: ACCOUNT_NAME@SERVER",
        )
        parser.add_argument(
            "--stat_func",
            default=OPT_STAT_FUNC,
            choices=StatFunc.get_stat_funcs(),
            help="Select how to calculate for ally/enemy performance: tank-tier stats, global player stats",
        )
        parser.add_argument(
            "-u", "--url", action="store_true", default=False, help="Print replay URLs"
        )
        parser.add_argument(
            "--tankfile",
            type=str,
            default="tanks.json",
            help='JSON file to read Tankopedia from. Default is "tanks.json"',
        )
        parser.add_argument(
            "--mapfile",
            type=str,
            default="maps.json",
            help='JSON file to read Blitz map names from. Default is "maps.json"',
        )
        parser.add_argument(
            "--json",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Export replay info in JSON format",
        )
        parser.add_argument(
            "-o",
            "--outfile",
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=OPT_DB,
            help="Use DB - You are unlikely to have it",
        )
        parser.add_argument(
            "--filters",
            type=str,
            default=None,
            help='Filter replays based on categories. Filters given in JSON format.\nUse array "[]" for multiple filters/values. see --mode help.\nExample: : [ {"tank_tier" : [8,9,10] }, { "player_wins" : 5 }]',
        )
        parser.add_argument(
            "--filters_db",
            type=str,
            default=None,
            help="[only for DB setup] Filter replays in DB based on categories. Filters given in MongoDB JSON format. See --mode help",
        )
        parser.add_argument(
            "--min",
            type=int,
            default=None,
            help="Only select replays from players with minimum number of replays in the dataset",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="*",
            help="Files/dirs to read. Use '-' for STDIN, \"db:\" for database",
        )
    ...
    try:
        wg = WG(
            WG_app_id=WG_APP_ID,
            tankopedia_fn=args.tankfile,
            maps_fn=args.mapfile,
            stats_cache=True,
        ...
        (player_stats, stat_id_map) = await process_player_stats(
            players, OPT_WORKERS_N, args, db
        )
...
async def export_csv(
    ...
    blt_cat_list: BattleCategorizationList,
...
async def export_json(
    ...
    try:
        ...
        if args.outfile == "-":
            ...
            export_file = args.outfile
        await bu.save_JSON(
            export_file, res, pretty=False
        )  ## Excel does not understand pretty JSON
        bu.verbose_std("Results exported to: " + export_file)
    except Exception as err:
        bu.error(exception=err)
...
async def help_extended(
    ...
    print(
        "\tThese filters let you filter only battles matching the filter. The filter has been proper JSON dict."
    )
    ...
    print("-------------------------------------------------------------------\n")
    print(
        'You are unlikely to have DB setup required for these filters. This is "Jylpah\'s special"'
    )
    print("")
    print(
        'Syntax: --filters_db \'{ "replay.param": VALUE, "replay.param2": { "$MONGO_OPERATOR" : VALUE }, ... }\''
    )
    print(
        '\tExample: --filters_db \'{ "data.summary.protagonist": ACCOUNT_ID, "data.summary.battle_start_timestamp": { "$gt" : 1602853200 } }\''
    )
    if db is not None:
        dbc = db[DB_C_REPLAYS]
        try:
...
def process_player_dist(results: list, player_stats: dict, stat_id_map: dict) -> dict:
    ...
    try:
        for field in histogram_fields:
            hist_stats[field] = PlayerHistogram(
                field,
                histogram_fields[field][0],
                histogram_fields[field][1],
                histogram_fields[field][2],
                histogram_fields[field][3],
            )
                ...
                if player_remapped in player_stats:
                    if player in result["allies"]:
                        for stat_field in hist_stats:
                            hist_stats[stat_field].record_ally(
                                player_stats[player_remapped][stat_field]
                            )
                    else:
                        for stat_field in hist_stats:
                            hist_stats[stat_field].record_enemy(
                                player_stats[player_remapped][stat_field]
                            )
...
def filter_results(results: list, filters_json: str, stats_filters=None) -> bool:
    ...
    try:
        ...
        filter_cats = BattleCategorizationList.get_categorizations_stats(
            filters.keys(), stats_filters
        )
...
def remap_stat_id(stat_id_map: dict, stat_id_remap: dict):
    bu.debug(
        "Remapping stat_ids: stat_id_map: "
        + str(len(stat_id_map))
        + " remap needed: "
        + str(len(stat_id_remap))
    )
...
def calc_team_stats(
    results: list, player_stats: dict, stat_id_map: dict, args: argparse.Namespace
) -> list:
        ...
        try:
            ...
            for ally in result["allies"]:
                ...
                for stat in stat_types:
                    if player_stats[ally_mapped][stat] is not None:
            ...
            for enemy in result["enemies"]:
                ...
                if enemy_mapped not in player_stats:
                    ...
                    continue
                for stat in stat_types:
                    if player_stats[enemy_mapped][stat] is not None:
            ...
            if player_mapped not in player_stats:
                ...
                for stat in stat_types:
                    if player_stats[player_mapped][stat] is not None:
            ...
            # Steamroller stats
            result["team_result"] = (
                str(result["allies_survived"]) + "-" + str(result["enemies_survived"])
            )
        ...
        while True:
            ...
            try:
                ...
                bu.debug(
                    "get_db_"
                    + args.stat_func
                    + "_stats returned: account_id="
                    + str(get_account_id_f_stat_id(stat_id))
                    + ": "
                    + str(stats[stat_id]),
                    workerID,
                )
                ...
                if stats[stat_id] is None:
                    ...
                    bu.debug(
                        ...
                        + args.stat_func
                        + "_stats returned: account_id="
                        + str(get_account_id_f_stat_id(pruned_stat_id))
                        + ": "
                        + str(stats[pruned_stat_id]),
                        workerID,
                    )
...
async def get_wg_tank_stats(stat_id_str: str, cache_only=False) -> dict:
    ...
    try:
        ...
        player_stats = await wg.get_player_tank_stats(
            account_id, [tank_id], hist_stats, cache_only=cache_only
        )
...
async def get_db_tank_stats(db: AsyncIOMotorDatabase, stat_id_str: str) -> dict:
    ...
    try:
        ...
        project["_id"] = False
        bu.debug(
            "find(): tank_id={} account_id={} last_battle_time>={}".format(
                tank_id, account_id, battletime - time_buffer
            )
        )
        cursor = (
            dbc.find(
                {
                    "$and": [
                        ...
                        {"last_battle_time": {"$gte": battletime - time_buffer}},
                    ]
                },
                projection=project,
            )
...
async def get_wg_tank_tier_stats(stat_id_str: str, cache_only: bool = False) -> dict:
    """Get player stats from WG. Returns WR per tier of tank_id"""
    global wg
    try:
        ...
        player_stats = await wg.get_player_tank_stats(
            account_id, tier_tanks, hist_stats, cache_only=cache_only
        )
        # bu.debug('account_id: ' + str(account_id) + ' ' + str(player_stats))
...
async def get_db_tank_tier_stats(db: AsyncIOMotorDatabase, stat_id_str: str) -> dict:
    """Get player stats from MongoDB (you are very unlikely to have it, unless you have set it up)"""
    global wg
    if db is None:
        return None
        ...
        pipeline = [
            {
                "$match": {
                    "$and": [
                        {"account_id": account_id},
                        {"last_battle_time": {"$lte": battletime + time_buffer}},
                        {"tank_id": {"$in": tier_tanks}},
                    ]
                }
            },
            {"$sort": {"last_battle_time": -1}},
...
async def get_db_tier_x_stats(db: AsyncIOMotorDatabase, stat_id_str: str) -> dict:
    """Get player stats from MongoDB (you are very unlikely to have it, unless you have set it up)"""
    if db is None:
        return None
    try:
...
async def get_db_player_stats(db: AsyncIOMotorDatabase, stat_id_str: str) -> dict:
    """Get player stats from MongoDB (you are very unlikely to have it, unless you have set it up)"""
    if db is None:
        return None
    try:
...
async def get_wg_player_stats(stat_id_str: str, cache_only=False) -> dict:
    ...
    try:
        ...
        player_stats = await wg.get_player_stats(
            account_id, hist_stats, cache_only=cache_only
        )
        # bu.debug('account_id: ' + str(account_id) + ' ' + str(player_stats))
...
async def mk_replayQ(
    ...
    if files[0] == "db:":
        ...
        try:
            dbc = db[DB_C_REPLAYS]
            cursor = None
            if args.filters_db is not None:
...
async def replay_reader(queue: asyncio.Queue, readerID: int, args: argparse.Namespace):
    ...
    try:
        while True:
            ...
            try:
                ...
                if replay_json is None:
                    bu.warning(
                        "Invalid replay. Skipping: "
                        + (replay_file if replay_file is not None else "")
                    )
                    queue.task_done()
                    continue
        ...
        bu.debug(
            str(len(results)) + " replays, " + str(len(playerstanks)) + " player/tanks",
            readerID,
        )
    ...
    try:
        ...
        for player in replay_summary["details"]:
            ...
            if player["dbid"] == account_id:
                ...
                tmp["tank_type"] = wg.get_tank_type(tank_id)
                tmp["tank_nation"] = wg.get_tank_nation(tank_id)
                tmp["is_premium"] = int(wg.is_premium(tank_id))
                platoon_ndx = player["squad_index"]
                tmp["squad_index"] = platoon_ndx
                tmp["in_platoon"] = 0 if platoon_ndx is None else 1
                ...
                if player["dbid"] in replay_summary["allies"]:
                    result["allies"].add(
                        get_stat_id(tmp_account_id, tmp_tank_id, tmp_battletime)
                    )
                    if survived:
                        result["allies_survived"] += 1
                else:
        ...
        result["player"] = get_stat_id(
            account_id, tank_id, result["battle_start_timestamp"]
        )
        bu.debug("Player stats_id: " + result["player"])
        # remove platoon buddy from stats
        if result["squad_index"] is not None:
            for player in replay_summary["details"]:
                bu.debug(str(player))
                if (
                    (player["squad_index"] == result["squad_index"])
                    and (player["dbid"] in replay_summary["allies"])
                    and (player["dbid"] != account_id)
                ):
                    # platoon buddy found
                    tmp_account_id = player["dbid"]
                    tmp_tank_id = player["vehicle_descr"]
        ...
        result["battle"] = (
            str(REPLAY_I)
            + ": "
            + replay_summary["vehicle"]
            + " @"
            + replay_summary["map_name"]
        )
        bu.debug(str(result))
        return result
    except KeyError as err:
