...
async def main(argv) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Post replays(s) to WoTinspector.com and retrieve replay data as JSON"
    )
    parser.add_argument(
        "-id", dest="accountID", type=int, default=WG_ID, help="WG account_id"
    )
    parser.add_argument(
        "-a",
        "--account",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Title for replays. Use "NN" for continous numbering. Default is automatic naming',
    )
    parser.add_argument(
        "-p",
        "--private",
    ...
    try:
        ...
        bu.verbose_std(
            str(REPLAY_N)
            + " replays: "
            + str(REPLAY_N - SKIPPED_N - ERROR_N)
            + " uploaded, "
            + str(SKIPPED_N)
            + " skipped, "
            + str(ERROR_N)
...
async def replayWorker(
    queue: asyncio.Queue, workerID: int, account_id: int, priv=False
):
    ...
    while True:
            ...
            if os.path.isfile(replay_json_fn):
                async with aiofiles.open(replay_json_fn) as fp:
                    ...
                    if wi.chk_JSON_replay(replay_json):
                        bu.verbose_std(
                            msg_str + title + " has already been posted. Skipping."
                        )
                    else:
                        os.remove(replay_json_fn)
                        bu.debug(
                            msg_str
                            + "Replay JSON not valid/complete: Deleting "
                            + replay_json_fn,
                            id=workerID,
                        )
                    SKIPPED_N += 1
        ...
        try:
            ...
            async with aiofiles.open(filename, "rb") as fp:
                ...
                json_resp = await wi.post_replay(
                    await fp.read(), filename, account_id, title, priv, N
                )
                if json_resp is not None:
                    if await bu.save_JSON(replay_json_fn, json_resp):
                        if wi.chk_JSON_replay(json_resp):
                            ...
                            bu.warning(
                                msg_str
                                + "Replay file is not valid/complete: "
                                + filename
                            )
                            ERROR_N += 1
                    else:
            ...
            bu.error(
                msg_str + "Unexpected Exception: " + str(type(err)) + " : " + str(err)
            )
            ...
            with zipfile.ZipFile(replayfile, "r") as archive:
                ...
                with io.TextIOWrapper(archive.open("meta.json")) as meta:
                    ...
                    try:
                        ...
                        tank_name = wg.get_tank_name(
                            metadata_json["vehicleCompDescriptor"]
                        )
                        map_name = wg.get_map(metadata_json["mapName"])
                        bu.debug(
                            "Tank: " + tank_name
                            if tank_name is not None
                            else "NOT FOUND IN tanks.json" + " Map: " + map_name
                        )
                    except Exception as err:
                        bu.error(exception=err)
